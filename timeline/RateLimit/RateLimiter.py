...
class RateLimiter:
    ...
    def __init__(self, debug, limits : (int,int) = (10,60), name: str =""):
    ...
    async def getToken(self):
        ...
        async with self.lock:
                ...
                #If count limit reached, await for the end of time window
                    ...
                    if self.debug:
                        print(self.name+" limit reached, sleeping for "+str( int((self.duration) - time.mktime(datetime.datetime.utcnow().timetuple())) + 1)+" seconds")
                        print("Limit : "+str(self.limit)+" per "+str(self.duration)+" / Count : "+ str(self.count))
                    #Await for the next time window
                    await asyncio.sleep( int((self.duration) - time.mktime(datetime.datetime.utcnow().timetuple())) + 1)
